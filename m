...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 8BDF9121A4E
	for <lists+linux-kselftest@lfdr.de>; Mon, 16 Dec 2019 21:00:29 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727403AbfLPT6M (ORCPT <rfc822;lists+linux-kselftest@lfdr.de>);
        Mon, 16 Dec 2019 14:58:12 -0500
Received: from hqnvemgate25.nvidia.com ([216.228.121.64]:12968 "EHLO
        hqnvemgate25.nvidia.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1727466AbfLPT6L (ORCPT
        <rfc822;linux-kselftest@vger.kernel.org>);
        Mon, 16 Dec 2019 14:58:11 -0500
Received: from hqpgpgate101.nvidia.com (Not Verified[216.228.121.13]) by hqnvemgate25.nvidia.com (using TLS: TLSv1.2, DES-CBC3-SHA)
        id <B5df7e1b90000>; Mon, 16 Dec 2019 11:57:45 -0800
Received: from hqmail.nvidia.com ([172.20.161.6])
  by hqpgpgate101.nvidia.com (PGP Universal service);
  Mon, 16 Dec 2019 11:57:53 -0800
X-PGP-Universal: processed;
        by hqpgpgate101.nvidia.com on Mon, 16 Dec 2019 11:57:53 -0800
Received: from HQMAIL111.nvidia.com (172.20.187.18) by HQMAIL111.nvidia.com
 (172.20.187.18) with Microsoft SMTP Server (TLS) id 15.0.1473.3; Mon, 16 Dec
 2019 19:57:52 +0000
Received: from rnnvemgw01.nvidia.com (10.128.109.123) by HQMAIL111.nvidia.com
 (172.20.187.18) with Microsoft SMTP Server (TLS) id 15.0.1473.3 via Frontend
 Transport; Mon, 16 Dec 2019 19:57:52 +0000
Received: from rcampbell-dev.nvidia.com (Not Verified[10.110.48.66]) by rnnvemgw01.nvidia.com with Trustwave SEG (v7,5,8,10121)
        id <B5df7e1bf0005>; Mon, 16 Dec 2019 11:57:52 -0800
...
Subject: [PATCH v5 2/2] mm/hmm/test: add self tests for HMM
Date:   Mon, 16 Dec 2019 11:57:33 -0800
Message-ID: <20191216195733.28353-3-rcampbell@nvidia.com>
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=nvidia.com; s=n1;
        t=1576526265; bh=NXF8Kx/Osb6jUtchFfrJKFmUwG5hU7uqhCL+He0bEUo=;
        h=X-PGP-Universal:From:To:CC:Subject:Date:Message-ID:X-Mailer:
         In-Reply-To:References:MIME-Version:X-NVConfidentiality:
         Content-Type:Content-Transfer-Encoding;
        b=
...
---
 ...
 10 files changed, 2861 insertions(+)
 create mode 100644 lib/test_hmm.c
 create mode 100644 tools/testing/selftests/vm/hmm-tests.c
 create mode 100755 tools/testing/selftests/vm/test_hmm.sh

diff --git a/MAINTAINERS b/MAINTAINERS
index 02d5278a4c9a..38cf016f8108 100644
--- a/MAINTAINERS
+++ b/MAINTAINERS
@@ -7508,7 +7508,10 @@ L:	linux-mm@kvack.org
 S:	Maintained
 F:	mm/hmm*
 F:	include/linux/hmm*
...
index d1842fe756d5..d3dbca0352c9 100644
--- a/lib/Kconfig.debug
+++ b/lib/Kconfig.debug
@@ -2117,6 +2117,17 @@ config TEST_MEMINIT
=20
 	  If unsure, say N.
=20
+config TEST_HMM
...
index 000000000000..5b65f397df83
--- /dev/null
+++ b/lib/test_hmm.c
@@ -0,0 +1,1367 @@
...
+	xa_init(&dmirror->pt);
+	dmirror->ranges =3D RB_ROOT_CACHED;
+
+	/*
...
+	for (node =3D interval_tree_iter_first(&dmirror->ranges, 0UL, ~0UL);
+	     node;
+	     node =3D next) {
+		next =3D interval_tree_iter_next(node, 0UL, ~0UL);
+		dmi =3D container_of(node, struct dmirror_interval, tree_node);
+		interval_tree_remove(&dmi->tree_node, &dmirror->ranges);
+		mmu_interval_notifier_remove(&dmi->notifier);
+	}
...
+	}
+
+	dmi->tree_node.start =3D start;
+	dmi->tree_node.last =3D last;
+	interval_tree_insert(&dmi->tree_node, &dmirror->ranges);
+
+	return dmi;
+}
+
...
+	interval_tree_remove(&dmi->tree_node, &dmirror->ranges);
+
+	if (dmi->tree_node.start >=3D range->start) {
+		if (dmi->tree_node.last < range->end) {
+			/* Remove the whole interval. */
+			mmu_interval_notifier_remove(mni);
+		} else {
+			/* Keep the right-hand part of the interval. */
+			mmu_interval_notifier_update(mni, range->end,
+				dmi->tree_node.last - range->end + 1);
+			dmi->tree_node.start =3D range->end;
+			interval_tree_insert(&dmi->tree_node,
+					     &dmirror->ranges);
+		}
+		return;
+	}
...
+	mmu_interval_notifier_update(mni, dmi->tree_node.start,
+				     range->start - dmi->tree_node.start);
+	last =3D dmi->tree_node.last;
+	dmi->tree_node.last =3D range->start - 1;
+	interval_tree_insert(&dmi->tree_node, &dmirror->ranges);
+
...
+}
+
+static bool dmirror_range_invalidate(struct mmu_interval_notifier *mni,
+				const struct mmu_notifier_range *range,
...
+
+	mutex_lock(&dmirror->mutex);
+	node =3D interval_tree_iter_first(&dmirror->ranges, range->start,
+					range->end - 1);
+	if (node) {
+		dmi =3D container_of(node, struct dmirror_interval, tree_node);
+		if (range->start >=3D node->start) {
+			if (range->end - 1 > node->last)
+				range->end =3D node->last + 1;
+			goto found;
+		}
+		if (range->end - 1 > node->last)
+			range->end =3D node->last + 1;
...
+	 */
+	vma =3D find_vma(dmirror->mm, range->start);
...
+	if (end < range->end)
+		range->end =3D end;
+	dmi =3D dmirror_new_interval(dmirror, vma->vm_start, end - 1);
+	if (!dmi) {
+		ret =3D -ENOMEM;
+		goto err;
+	}
...
ts/vm/hmm-tests.c
new file mode 100644
index 000000000000..bbf5cdcdc7da
--- /dev/null
