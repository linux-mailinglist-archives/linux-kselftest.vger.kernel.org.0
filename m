...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 8323273EFFC
	for <lists+linux-kselftest@lfdr.de>; Tue, 27 Jun 2023 02:58:37 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230093AbjF0A6b (ORCPT <rfc822;lists+linux-kselftest@lfdr.de>);
        Mon, 26 Jun 2023 20:58:31 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:54692 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229992AbjF0A60 (ORCPT
        <rfc822;linux-kselftest@vger.kernel.org>);
        Mon, 26 Jun 2023 20:58:26 -0400
Received: from us-smtp-delivery-124.mimecast.com (us-smtp-delivery-124.mimecast.com [170.10.129.124])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id C7B861BCE
...
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        ...
        bh=bNpWLXRI+o8miy9SxFN6kSeceaZ+bGWmmB1VsrAO6qI=;
        b=UnR62i71J36eBK1JieYg4zyAM3G6pGW/gakObYKSEvFvg/ML03Op1Q/dMg24PR0E1gZb8b
        Scxor0sQB1Hj2gV6DOhP0t2CDdYErAXx7G4G4Mip01NS2HFGeU6R06l4tk8BKO11/JWhUd
        LrCJK0f6Uj3kHbWCTLlOwYDSyaCsxVI=
Received: from mimecast-mx02.redhat.com (mimecast-mx02.redhat.com
 [66.187.233.88]) by relay.mimecast.com with ESMTP with STARTTLS
 (version=TLSv1.2, cipher=TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384) id
 us-mta-607-qhiXYFOkM7KWf8BdyiJWww-1; Mon, 26 Jun 2023 20:56:03 -0400
X-MC-Unique: qhiXYFOkM7KWf8BdyiJWww-1
Received: from smtp.corp.redhat.com (int-mx06.intmail.prod.int.rdu2.redhat.com [10.11.54.6])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx02.redhat.com (Postfix) with ESMTPS id 030DC891F22;
        Tue, 27 Jun 2023 00:56:03 +0000 (UTC)
Received: from llong.com (unknown [10.22.33.159])
        by smtp.corp.redhat.com (Postfix) with ESMTP id 18D3C2166B25;
...
Subject: [PATCH v3 5/9] cgroup/cpuset: Add cpuset.cpus.exclusive for v2
Date:   Mon, 26 Jun 2023 20:55:25 -0400
Message-Id: <20230627005529.1564984-6-longman@redhat.com>
...
---
 kernel/cgroup/cpuset.c | 738 ++++++++++++++++++++++++-----------------
 1 file changed, 434 insertions(+), 304 deletions(-)

diff --git a/kernel/cgroup/cpuset.c b/kernel/cgroup/cpuset.c
index 64f9e305b3ab..d841550d9348 100644
...
-	} else {
+	if (is_partition_valid(cs))
+		cpumask_and(new_cpus, cs->exclusive_cpus, cpu_active_mask);
+	else
...
@@ -1323,8 +1350,39 @@ static void update_partition_sd_lb(struct cpuset *cs, int old_prs)
...
@@ -1826,6 +1986,7 @@ static int update_cpumask(struct cpuset *cs, struct cpuset *trialcs,
 {
 	int retval;
 	struct tmpmasks tmp;
+	struct cpuset *parent = parent_cs(cs);
...
@@ -1841,6 +2002,7 @@ static int update_cpumask(struct cpuset *cs, struct cpuset *trialcs,
 	 */
 	if (!*buf) {
...
+		cpumask_clear(trialcs->exclusive_cpus);
 		...
 		if (retval < 0)
@@ -1849,6 +2011,13 @@ static int update_cpumask(struct cpuset *cs, struct cpuset *trialcs,
...
@@ -1858,11 +2027,21 @@ static int update_cpumask(struct cpuset *cs, struct cpuset *trialcs,
...
@@ -1873,69 +2052,44 @@ static int update_cpumask(struct cpuset *cs, struct cpuset *trialcs,
 		 */
 		invalidate = true;
 		rcu_read_lock();
-		parent = parent_cs(cs);
...
+		seq_printf(sf, "%*pbl\n", cpumask_pr_args(subpartitions_cpus));
 	...
 	default:
 		ret = -EINVAL;
@@ -3186,11 +3342,18 @@ static struct cftype dfl_files[] = {
...
+		.flags = CFTYPE_ONLY_ON_ROOT | CFTYPE_DEBUG,
 	...
 	{ }	/* terminate */
@@ -3364,6 +3527,7 @@ static void cpuset_bind(struct cgroup_subsys_state *root_css)
...
@@ -3502,11 +3666,13 @@ int __init cpuset_init(void)
 {
...
@@ -3647,30 +3813,15 @@ static void cpuset_hotplug_update_tasks(struct cpuset *cs, struct tmpmasks *tmp)
